...
import { BinaryNode } from './decode';

// Função para decodificar JID (adaptada do jid-utils.js)
interface JidDecodedResult {
    domainType?: number;
    device?: number;
    user: string;
    server: string;
...
const jidDecode = (jid: string): JidDecodedResult | null => {
    ...
    if (colonIdx >= 0) {
        return {
            ...
            device: parseInt(user.slice(colonIdx + 1)),
            server
        };
    }
...
// Criar mapa de tokens para busca rápida
const createTokenMap = () => {
    const map: { [key: string]: { dict?: number; index: number } } = {};
    ...
    // Double byte tokens
    DOUBLE_BYTE_TOKENS.forEach((row, dictIndex) => {
        if (row) {
            row.forEach((token, tokenIndex) => {
                if (token) {
                    map[token] = { dict: dictIndex, index: tokenIndex };
                }
            });
        }
    });
...
export const encodeBinaryNode = (node: BinaryNode, opts = { TAGS, TOKEN_MAP }, buffer: number[] = [0]): Buffer => {
...
const encodeBinaryNodeInner = (
    ...
    const pushBytes = (bytes: Buffer | Uint8Array | number[]) => {
    ...
    const writeJid = ({ domainType, device, user, server }: JidDecodedResult) => {
    ...
    if (typeof content === 'string') {
        ...
        const validContent = content.filter(item => 
            item && (item.tag || Buffer.isBuffer(item) || item instanceof Uint8Array || typeof item === 'string')
        );
...
// Classe WABinaryEncoder para compatibilidade
export class WABinaryEncoder {
    static encode = encodeBinaryNode;
}